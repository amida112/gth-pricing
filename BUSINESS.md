...
```
Kiện nguyên  →  Kiện lẻ  →  Đã bán
     ↕              ↕
   (nhập kho)   (bán 1 phần)   (bán hết / remaining ≤ 0)
```

| Trạng thái | Ý nghĩa | Màu |
|------------|---------|-----|
| Kiện nguyên | Chưa bán tấm nào (`remainingBoards >= boardCount`) | Xanh lá |
| Kiện lẻ | Đã bán một phần tấm (`0 < remainingBoards < boardCount`) | Cam |
| Đã bán | Hết hàng (`remainingBoards ≤ 0`) | Nâu |

**Cơ chế trừ kho — trừ ngay lập tức**:
- Khi nhân viên thêm kiện vào danh sách sản phẩm đơn hàng → `deductBundle()` trừ `remaining_boards` và `remaining_volume` **ngay lập tức**, bất kể đơn là nháp hay đã xác nhận.
- Khi gỡ kiện khỏi danh sách → `restoreBundle()` cộng lại ngay.
- Khi sửa số lượng/khối lượng → tính chênh lệch (delta), trừ/cộng ngay.
- Khi lưu đơn → **không làm gì với kho** (đã trừ trước đó).
- Cho phép tồn kho âm (nghiệp vụ hợp lệ: kiện thực tế có thể nhiều tấm hơn ghi nhận).

**Chống bán trùng**: Supabase realtime subscription trên `wood_bundles` (App.js) — khi kiện bị trừ ở tab A, tất cả tab khác nhận cập nhật ngay (~50-200ms) → BundleSelector tự loại kiện hết hàng.

**Cleanup khi rời trang không lưu (đơn mới)**: `deleteOrder()` đọc `order_items` → cộng lại remaining cho từng kiện. Nếu crash/reload → `cleanupStaleDrafts` dọn đơn nháp rác (throttle 1 lần/giờ) cũng qua `deleteOrder()`.

**Rollback edit mode**: Khi sửa đơn đã lưu, mỗi thay đổi (thêm/gỡ kiện) được track trong `editChangesRef`. Nếu đóng không lưu → reverse tất cả delta.
...
**Giải pháp**: Tồn kho trừ ngay khi thêm vào đơn + Supabase realtime tự động cập nhật danh sách kiện trên tất cả tab. Kiện hết hàng biến mất khỏi BundleSelector ngay lập tức (~50-200ms). Trường hợp hiếm (2 user trừ trong cùng 200ms) → tồn kho âm → cho phép (nghiệp vụ hợp lệ).
...
```
Mở form tạo đơn → hệ thống tạo đơn nháp trống (DB)
  → Chọn khách hàng (tìm kiếm bỏ dấu, tạo nhanh với cảnh báo trùng)
  → Mở BundlePicker chọn kiện gỗ
    → Filter theo loại gỗ, thuộc tính (chỉ hiện kiện còn tồn)
    → Tick chọn → kho trừ ngay lập tức (deductBundle)
    → Hệ thống tự tra giá cho từng kiện
  → Thêm dịch vụ (xẻ sấy, vận chuyển...)
  → Nhập phí ship, đặt cọc, công nợ
  → Lưu đơn hàng (kho đã trừ, chỉ lưu metadata)
  → Rời trang không lưu → kho cộng lại (deleteOrder restore)
```

**Đơn nháp tự động**: Khi mở form tạo đơn, hệ thống tạo đơn nháp trong DB ngay (`createDraftOrder`) để có ID. Items tự lưu mỗi 300ms (`saveDraftItems`). Nếu rời trang không lưu → đơn nháp bị xóa + kho hoàn trả. Nếu crash/reload → `cleanupStaleDrafts` dọn sau 1 giờ.

**Tìm kiếm khách hàng**: Hỗ trợ bỏ dấu (gõ "ha" tìm "Hà"), multi-token ("ha canh" tìm "Hà · Canh Nậu"), tìm theo tên, danh xưng, địa chỉ thường gọi, SĐT. Khi tạo khách nhanh → cảnh báo nếu trùng SĐT hoặc tên+nickname.
...
- Khi chọn khách hàng có đơn nợ cũ → hiện banner cảnh báo:
  ...
  - Bảng chi tiết từng đơn nợ: mã đơn, ngày bán, tổng, đã trả, còn nợ, số ngày
  - Đơn quá hạn (vượt `debtDays`) highlight đỏ
  - "Tổng nợ sau đơn này" = nợ cũ + tổng đơn mới

### 4.6c Ngày bán hàng (sale_date) vs Ngày tạo đơn (created_at)

**Bài toán**: Nhân viên thường nhập đơn hàng vào hệ thống sau khi bán xong vài ngày (nhập đuổi). Nếu chỉ có một ngày duy nhất, ngày trên đơn không phản ánh đúng thực tế bán hàng → lịch sử mua sai thứ tự, công nợ tính sai số ngày quá hạn, đối chiếu kho lệch.

**Giải pháp**: Tách 2 ngày riêng biệt trên đơn:
- **`sale_date`** (Ngày bán hàng): Ngày nhân viên thực sự bán cho khách. Mặc định = lúc tạo đơn, sửa được trên form. Dùng cho mọi báo cáo nghiệp vụ.
- **`created_at`** (Ngày tạo dòng): Ngày dòng dữ liệu được ghi vào hệ thống. Không sửa được. Dùng cho audit và sinh mã đơn.

**Ví dụ**: Khách mua hàng ngày 25/03, đến 29/04 nhân viên mới ngồi nhập máy:
- `sale_date = 25/03` (trên form chọn lại)
- `created_at = 29/04` (auto)
- `order_code = DH-20260429-001` (theo ngày nhập)

**Logic dùng `sale_date`**:
- Sắp xếp danh sách đơn (mặc định)
- Lịch sử mua khách hàng
- Công nợ aging — số ngày quá hạn = today − sale_date (đơn 25/03 → quá hạn 35 ngày)
- In hóa đơn — label "Ngày bán"
- Filter "Đối chiếu kho sau 20/3" — đơn có sale_date sau 20/3

**Logic giữ `created_at`**:
- Mã đơn `DH-YYYYMMDD-NNN` (theo ngày nhập, dễ truy ngược)
- Dọn nháp rác > 1 giờ
- Doanh thu Dashboard (đã dùng `payment_date`)
- Audit log + lịch sử sửa đơn

**Hiển thị UI khi nhập đuổi**:
- Cột "Ngày bán" trong DS đơn có icon `↶` khi `sale_date` lệch `created_at` > 1 ngày
- Hover xem ngày tạo đơn thật
- Form sửa đơn: dòng phụ "Tạo lúc: ..." dưới ô ngày bán
- In hóa đơn: dòng nhỏ "Tạo đơn lúc: ..." dưới "Ngày bán"
...
- `fetchCustomerDebtDetail()`: chi tiết từng đơn nợ (orderId, orderCode, saleDate, createdAt, totalAmount, debt, totalPaid, outstanding, daysSince)
...
- **Thời gian nợ**: tính từ `sale_date` (ngày bán thực tế) của đơn nợ cũ nhất chưa tất toán. Quá `debtDays` → highlight đỏ "Quá hạn". Xem 4.6c về sale_date vs created_at.
