              ...
              return (
                <tr key={r._id}>
                  ...
                  <td style={{ padding: "3px 4px" }}>
                    <input value={r.quality} onChange={e => upd("quality", e.target.value)} placeholder="CL" style={{ ...iS, width: 52 }} />
                  </td>
            ...
            {pieces.map((p, i) => {
              ...
              return (
                <tr key={p.id} style={{ background: i % 2 ? "var(--bgs)" : "#fff" }}>
                  ...
                  {/* Chất lượng — free text cho mọi loại gỗ */}
                  <td style={tdS}>
                    {ce
                      ? <input value={p.quality || ""} onChange={e => updateStatus(p.id, "quality", e.target.value)}
                          placeholder="CL" style={{ padding: "2px 5px", borderRadius: 4, border: "1px solid var(--bd)", fontSize: "0.68rem", fontWeight: 600, outline: "none", width: 50, background: "var(--bgc)", color: "var(--tp)" }} />
